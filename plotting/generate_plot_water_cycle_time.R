...
query <- "SELECT sum_date, avg_cycle_time, median_cycle_time, rolling_avg_cycle_time_90d, rolling_median_cycle_time_90d, lifetime_median_cycle_time FROM (SELECT sum_date, avg_cycle_time, median_cycle_time, rolling_avg_cycle_time_90d, rolling_median_cycle_time_90d, lifetime_median_cycle_time FROM water_statistics.water_statistics_daily_cycle_view ORDER BY sum_date DESC LIMIT 90) x ORDER BY sum_date;"
...
png(filename=fname, width=1024, height=400, units='px', pointsize=12, bg='white')
plotmin <- floor(min(res$median_cycle_time, res$rolling_median_cycle_time_90d, res$lifetime_median_cycle_time, na.rm=TRUE))
plotmax <- ceiling(max(res$median_cycle_time, res$rolling_median_cycle_time_90d, res$lifetime_median_cycle_time, na.rm=TRUE))
vseq <- seq(plotmin, plotmax, 1)
...
lines(res$sum_date, res$rolling_median_cycle_time_90d, col='orange')

...
query <- "SELECT u.hour AS label, u.kwh, s.kwh_avg, u.complete FROM electricity_usage_hourly u INNER JOIN electricity_statistics_hourly s ON u.hour=s.hour WHERE NOT u.hour = date_part('hour', CURRENT_TIMESTAMP) ORDER BY u.updated;"

...
colors <- colorRampPalette(c(cycle_panel, '#dbeafe', cycle_blue, '#1e3a8a'))(20)
...
axis(side=1, at=seq(0,23,2), labels=seq(0,23,2), col=NA, col.ticks=cycle_axis, col.axis=cycle_axis)
cycle_axis_y(0:6, c('Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'))
text(expand.grid(0:23, 0:6), labels=ifelse(as.vector(counts) > 0, as.vector(counts), ''), cex=0.65, col=cycle_text)
dev.off()

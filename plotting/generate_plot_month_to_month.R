...
#query <- "SELECT DATE_TRUNC('month', CURRENT_TIMESTAMP - interval '4 hours' + interval '1 month') - interval '1 second' AS xmax;"
...
query <- "SELECT kwh_avg FROM electricity_statistics_monthly WHERE month = date_part('month', CURRENT_TIMESTAMP - interval '4 hours');"
kwhavg <- dbGetQuery(con, query)

query <- "SELECT time, CASE WHEN minuteh IS NULL THEN minute ELSE minuteh END AS minute FROM prediction_test WHERE date_part('year', time) = date_part('year', CURRENT_TIMESTAMP - interval '4 hours') AND date_part('month', time) = date_part('month', CURRENT_TIMESTAMP - interval '4 hours') AND minute > 0 ORDER BY time;"

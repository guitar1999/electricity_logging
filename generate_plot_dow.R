...
query2 <- "SELECT day_of_week AS label, date_part('dow', CURRENT_TIMESTAMP) AS dow, increment_usage('electricity_usage_dow', 'dow') AS kwh, kwh_avg, complete FROM electricity_usage_dow WHERE dow = date_part('dow', CURRENT_TIMESTAMP);"

...
query <- "SELECT doy, kwh, kwh_avg, complete FROM electricity_usage_doy WHERE NOT doy = date_part('doy', CURRENT_TIMESTAMP) AND doy >= date_part('doy', CURRENT_TIMESTAMP - interval '29 days') AND NOT timestamp IS NULL ORDER BY timestamp;"

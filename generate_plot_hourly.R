...
query2 <- "SELECT date_part('hour', CURRENT_TIMESTAMP) AS label, increment_usage('electricity_usage_hourly', 'hour') AS kwh, kwh_avg, complete FROM electricity_usage_hourly WHERE hour = date_part('hour', CURRENT_TIMESTAMP);"

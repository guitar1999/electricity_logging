...
query2 <- "SELECT date_part('hour', CURRENT_TIMESTAMP - interval '4 hours') AS label, u.kwh, s.kwh_avg_dow_season AS kwh_avg, complete FROM electricity_usage_hourly u INNER JOIN energy_statistics.electricity_statistics_hourly_dow_season s ON u.hour=s.hour WHERE u.hour = date_part('hour', CURRENT_TIMESTAMP - interval '4 hours') AND s.season = (SELECT season FROM meteorological_season WHERE doy = date_part('doy', CURRENT_TIMESTAMP - interval '4 hours')) AND dow = date_part('dow', CURRENT_TIMESTAMP - interval '4 hours');"
...
fname <- '/var/www/electricity/hourly_dow_season.png'
title <- "Electricity Used in the Last Day (Seasonal DOW Averaging)"
label.x <- "Hour"
label.y <- "kwh"

...
query <- "SELECT u.hour AS label, u.kwh, s.kwh_avg_dow_season AS kwh_avg, u.complete FROM electricity_usage_hourly u INNER JOIN energy_statistics.electricity_statistics_hourly_dow_season s ON u.hour=s.hour WHERE NOT u.hour = date_part('hour', CURRENT_TIMESTAMP) AND s.season = (SELECT season FROM meteorological_season WHERE doy = date_part('doy', CURRENT_TIMESTAMP)) AND s.dow = date_part('dow', CURRENT_TIMESTAMP) ORDER BY u.timestamp;"
...
query2 <- "SELECT date_part('hour', CURRENT_TIMESTAMP) AS label, u.kwh, s.kwh_avg_dow_season AS kwh_avg, complete FROM electricity_usage_hourly u INNER JOIN energy_statistics.electricity_statistics_hourly_dow_season s ON u.hour=s.hour WHERE u.hour = date_part('hour', CURRENT_TIMESTAMP) AND s.season = (SELECT season FROM meteorological_season WHERE doy = date_part('doy', CURRENT_TIMESTAMP)) AND dow = date_part('dow', CURRENT_TIMESTAMP);"

...
query <- "SELECT month, kwh, kwh_avg, complete FROM electricity_usage_monthly WHERE NOT month = date_part('month', CURRENT_TIMESTAMP) AND NOT timestamp IS NULL ORDER BY timestamp;"
res <- dbGetQuery(con, query)

query2 <- "SELECT date_part('month', CURRENT_TIMESTAMP) AS month, akwh AS kwh, kwh_avg, 'no'::text AS complete FROM (SELECT SUM((watts_ch1 + watts_ch2) * tdiff / 60 / 60 / 1000.) AS akwh FROM electricity_measurements WHERE measurement_time > CURRENT_TIMESTAMP - interval '1 month' AND date_part('month', measurement_time) = date_part('month', CURRENT_TIMESTAMP)) AS x, electricity_usage_monthly WHERE month = date_part('month', CURRENT_TIMESTAMP);"

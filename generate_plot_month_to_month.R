...
#prediction <- rbind(prediction, setNames(data.frame(max(last_year$measurement_time), prediction$minute[length(prediction$minute)]), names(prediction)))
#predline <- rbind(current_year[dim(current_year)[1],2:3], setNames(data.frame(prediction[dim(prediction)[1],]), names(current_year)[2:3]))
...
plot(last_year$measurement_time, last_year$cumulative_kwh, type='l', col='grey', lwd=1.5, ylim=c(0,pmax), xlab='', ylab='Cumulative kwh')
lines(current_year$measurement_time, current_year$cumulative_kwh, col='red', lwd=1.5)
